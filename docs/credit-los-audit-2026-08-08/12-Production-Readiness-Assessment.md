...
| The release gate is `npm run test:release` | **It never terminated.** The backend suite passed in 7.1s (108 suites / 1256 tests) and Jest then hung on open handles — 1h40m before manual kill. Any CI job invoking it would have hit its wall-clock limit and reported failure regardless of results. | Fixed — the gate returns on its own with `EXITCODE=0` in 39.95s: **230 suites / 2387 tests pass** (Phase 8b, 2026-08-10). The "10 failed tests / 7 failed suites" recorded in the middle column of this table's earlier revision predated `e2afc1d`, which fixed them. Jest still needs `forceExit` to release ~3 Redis + 17 Postgres sockets held inside Prisma's engine and ioredis; that is a recorded non-blocking test-infra limitation, not a product defect. **Open:** 1 of 2 consecutive gate runs hit 2 `socket hang up` flakes in `crm-authz.integration.test.ts` — determinism is not yet proven. Evidence: `docs/superpowers/plans/2026-08-10-credit-los-phase8b-evidence.md`. |
...
| `npm test` (backend, full suite) | **230 suites, 2387 tests, 0 failures**, `EXITCODE=0`, no surviving process. Requires `forceExit` (set in `jest.config.ts`) to release Prisma-engine and ioredis sockets after the green summary. |
| `npx jest src/credit --runInBand` | 108 suites, 1256 tests, 0 failures (Phase 8a measurement; superseded as current state by the full-suite row above) |
...
| `npm run test:release` (backend) | **`EXITCODE=0` in 39.95s wall clock** — seed → chain verification → P0 regression → full suite (230 suites / 2387 tests). Returns on its own. Caveat: an immediately preceding run of the same command failed with 2 `socket hang up` flakes in `crm-authz.integration.test.ts` (2385 passed); that suite passes 38/38 in isolation. Determinism is an open item. |
