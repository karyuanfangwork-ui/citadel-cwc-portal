...
| LOS-009 | Audit atomicity | Submit/score/approve | Several business writes commit before chain append. | Critical mutation and audit event commit or roll back together. | approval, scoring, document and transition services | Error may be shown after action succeeded; retry ambiguity. | Missing decision evidence/double action. | High | Pass transaction client to audit append and include business mutation in same transaction; add failure injection tests. | Yes—AuditChain transaction support | M | P0 | LOS-001 |
...
| LOS-013 | Audit chain | All critical events | Hash chain detects edits, but concurrent appends can select same previous hash; DB update prohibition unproven. | Serialized append-only chain with DB-enforced immutability. | `auditChain.service.ts`; migrations | Integrity verification may fork/fail. | Audit evidence challenged. | High | Per-application lock/sequence; DB trigger/privilege; scheduled verify and alert. | Yes | M | P1 | DBA deployment |
...
| LOS-015 | Return workflow | Return → amend → resubmit | Backend return reason optional; no reviewer change diff. | Mandatory reason, scoped amendment, before/after diff and renewed freeze. | approval validator; `REFERRED_BACK` transitions | Rework is unclear; reviewer repeats full review. | Unreviewed changes can slip through. | High | Mandate reason; derive diff from audit; always re-run readiness/freeze/memo on resume. | Yes—audit/versions | M | P1 | LOS-001, LOS-009 |
...
| LOS-018 | Child concurrency | Assessment edits | Root has optimistic version; most child records do not. | Detect conflicting edits and prevent last-write loss. | application `version`; financial/risk/document child services | Analysts overwrite each other. | Incorrect assessment basis. | Medium | Add version/updatedAt preconditions for high-risk child mutations. | Partial | M | P1 | API contract |
...
| LOS-020 | Inbox | Management queue | My Approvals can use general state list, showing non-actionable cases. | Authority/SOD-filtered backend inbox is single source. | `MyApprovals.tsx`; dashboard inbox service/tests | Noise and failed actions. | Operational mistakes/delay. | Medium | Connect page to existing inbox endpoint and explain ineligibility. | Yes | S | P1 | LOS-012 |
| LOS-021 | Integration | Final decision/disbursement | Default CBS adapter simulates booking; bureau/AML/OCR/e-sign may be placeholders. | Production blocks functions requiring an unconfigured live adapter or clearly operates record-only. | adapter registry/integration status | Staff may mistake simulation for external completion. | Operational/financial mismatch. | Critical for live booking | Fail closed by capability; production deployment evidence for each adapter. | Yes—registry/status | M + external | P0 if live lending | Vendor/configuration |
| LOS-022 | Test assurance | Whole journey | Strong unit/service tests; no credit browser E2E and DB suite unavailable in audit environment. | Seeded, repeatable individual/SME/corporate/control E2E release suite. | test inventory and audit execution | Defects emerge only during staff use. | Control-path regressions undetected. | High | Add focused browser/API tests from report 10; make DB service part of CI. | Yes—fixtures/tests | L | P1 | Stable test DB |
...
- P0: LOS-009 remains, plus LOS-021 when the deployment performs real lending/disbursement. (LOS-001 through LOS-008, LOS-010, LOS-011, LOS-012 closed 2026-08-08.)
- P1: LOS-013, LOS-015 through LOS-018, LOS-020, LOS-022. (LOS-014, LOS-019 closed 2026-08-08.)
...
## Follow-ups surfaced during Phase 4

- No admin route writes `CreditRuleConfig`, so missing-data policy overrides are
  seed/migration-only. An admin surface for policy parameters is P1.
- `scoringService.overrideScore` remains in the codebase but is no longer routed.
