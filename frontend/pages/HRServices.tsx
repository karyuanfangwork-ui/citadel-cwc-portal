...
const HRServices = () => {
  ...
  const fetchServiceDeskData = async () => {
    try {
      ...
      const hrDesk = desks.find((d: ServiceDesk) => d.code === 'HR');

      if (hrDesk) {
        setServiceDesk(hrDesk);
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-6 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {categories.length === 0 ? (
          ...
          categories.map((category) => (
            <Link
              key={category.id}
              to={`/hr/${serviceDesk.id}/create/${category.id}`}
              className="group p-6 bg-white border border-gray-100 rounded-xl hover:shadow-lg hover:border-emerald-600/20 transition-all"
            >
      ...
      <div className="mt-16 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-100 rounded-2xl p-8">
