import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import { serviceDeskService } from '../src/services/serviceDesk.service';
import SkeletonCategoryCard from '../src/components/SkeletonCategoryCard';
import { friendlyMessage } from '../src/utils/errorMessages';

interface ServiceDesk {
  id: string;
  name: string;
  code: string;
  description: string | null;
}

interface Category {
  id: string;
  name: string;
  description: string | null;
  icon: string | null;
  displayOrder: number;
}

const GroupFinance = () => {
  const [serviceDesk, setServiceDesk] = useState<ServiceDesk | null>(null);
  const [categories, setCategories] = useState<Category[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetchServiceDeskData();
  }, []);

  const fetchServiceDeskData = async () => {
    try {
      setLoading(true);
      setError(null);

      const desks = await serviceDeskService.getAllServiceDesks();
      const financeDesk = desks.find((d: ServiceDesk) => d.code === 'FINANCE');

      if (financeDesk) {
        setServiceDesk(financeDesk);
        const cats = await serviceDeskService.getCategories(financeDesk.id);
        setCategories(cats);
      } else {
        setError('Group Finance service desk not found');
      }
    } catch (err: any) {
      console.error('Error fetching service desk:', err);
      setError(friendlyMessage(err, 'Unable to load Group Finance categories. Please try again.'));
    } finally {
      setLoading(false);
    }
  };

  const getIconForCategory = (categoryName: string) => {
    const iconMap: Record<string, string> = {
      'Expense Reimbursement': 'receipt_long',
      'Procurement': 'shopping_cart',
      'Vendor Management': 'business',
      'Budget': 'account_balance',
      'Invoicing': 'description',
      'General Finance': 'payments',
    };
    return iconMap[categoryName] || 'help';
  };

  if (loading) {
    return (
      <div className="max-w-[1440px] mx-auto px-6 py-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {Array.from({ length: 6 }).map((_, i) => (
            <SkeletonCategoryCard key={i} />
          ))}
        </div>
      </div>
    );
  }

  if (error || !serviceDesk) {
    return (
      <div className="max-w-[1440px] mx-auto px-6 py-8">
        <div className="bg-red-50 border border-red-200 text-red-700 px-6 py-4 rounded-lg">
          <p className="font-semibold">Error loading service desk</p>
          <p className="text-sm">{error}</p>
        </div>
      </div>
    );
  }

  return (
    <div className="max-w-[1440px] mx-auto px-6 py-8">
      <nav className="flex items-center gap-2 mb-8 text-sm font-medium text-[#44546f]">
        <Link to="/" className="hover:text-amber-600">
          CWC
        </Link>
        <span className="material-symbols-outlined text-sm">chevron_right</span>
        <span className="text-[#101418] font-bold">{serviceDesk.name}</span>
      </nav>

      <div className="mb-12">
        <h1 className="text-4xl font-black text-[#101418] mb-4">{serviceDesk.name}</h1>
        <p className="text-lg text-[#44546f] max-w-3xl">
          {serviceDesk.description || 'Get help with finance-related requests and services'}
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {categories.length === 0 ? (
          <div className="col-span-full text-center py-12 text-[#44546f]">
            <p>No categories available</p>
          </div>
        ) : (
          categories.map((category) => (
            <Link
              key={category.id}
              to={`/finance/${serviceDesk.id}/create/${category.id}`}
              className="group p-6 bg-white border border-gray-100 rounded-xl hover:shadow-lg hover:border-amber-600/20 transition-all"
            >
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 bg-amber-50 text-amber-600 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform">
                  <span className="material-symbols-outlined text-2xl">
                    {category.icon || getIconForCategory(category.name)}
                  </span>
                </div>
                <div className="flex-1">
                  <h3 className="font-bold text-lg mb-2 group-hover:text-amber-600 transition-colors">
                    {category.name}
                  </h3>
                  <p className="text-sm text-[#44546f] leading-relaxed">
                    {category.description || 'Click to view available services'}
                  </p>
                </div>
              </div>
            </Link>
          ))
        )}
      </div>

      <div className="mt-16 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-100 rounded-2xl p-8">
        <div className="flex items-center gap-6">
          <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center text-amber-600">
            <span className="material-symbols-outlined text-3xl">support_agent</span>
          </div>
          <div className="flex-1">
            <h3 className="font-bold text-xl mb-2">Need finance assistance?</h3>
            <p className="text-[#44546f]">
              Our finance team is available Monday-Friday, 9 AM - 5 PM
            </p>
          </div>
          <button className="px-8 py-3 bg-amber-600 text-white font-bold rounded-xl hover:bg-amber-700 transition-colors">
            Contact Finance
          </button>
        </div>
      </div>
    </div>
  );
};

export default GroupFinance;
