...
export default function WorkflowDesigner() {
  ...
  const publish = async (statusRemap: Record<string, string>) => { if (!versionId) return; setLifecycleBusy(true); try { await workflowVersionService.publishVersion(versionId, statusRemap); setPublishOpen(false); await load(); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to publish workflow'); } finally { setLifecycleBusy(false); } };
  ...
  return <section className="flex min-h-[calc(100vh-8rem)] flex-col bg-[#f7f9fc]" data-testid="workflow-designer-page">
    ...
    {publishOpen && <PublishDialog workflow={workflow} version={detail.version} blocking={graph.blockingFindings} warnings={graph.warnings} remapPlan={graph.remapPlan} nodes={detail.graph.nodes} busy={lifecycleBusy} onClose={() => setPublishOpen(false)} onConfirm={(statusRemap) => void publish(statusRemap)} />}
  </section>;
}
