...
const SoftwareProvisionedModal: React.FC<SoftwareProvisionedModalProps> = ({ requestId, onSuccess, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      setError(null);
      await itWorkflowService.markSoftwareProvisioned(requestId, { provisioningNotes: provisioningNotes || undefined });
      onSuccess();
    } catch (err: any) {
      setError(err.response?.data?.error || 'Failed to mark as software provisioned');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center gap-3 p-5 border-b border-gray-100">
          <div className="size-9 rounded-lg bg-purple-100 flex items-center justify-center">
            <span className="material-symbols-outlined text-purple-600">settings_applications</span>
          </div>
          <div>
            <h2 className="font-bold text-base text-gray-900">Mark Software Provisioned</h2>
            <p className="text-xs text-gray-500">IT Workflow · Confirm software setup complete</p>
          </div>
        </div>
        <form onSubmit={handleSubmit}>
          <div className="p-5 space-y-4">
            <div>
              ...
              <textarea
                ...
                rows={3}
                placeholder="e.g. software installed, configuration completed, asset tag assigned"
          ...
          <div className="flex justify-end gap-2 p-5 border-t border-gray-100 bg-gray-50 rounded-b-2xl">
            ...
            <button type="submit" disabled={submitting} className="px-4 py-3 text-sm font-bold text-white bg-purple-600 rounded-lg hover:bg-purple-700 disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-600 focus-visible:ring-offset-2">
              {submitting ? 'Saving…' : 'Mark Provisioned'}
            </button>
          </div>
        </form>
      </div>
