...
interface RouteToCEOHRModalProps {
  requestId: string;
  onSuccess: () => void;
  onClose: () => void;
}

const RouteToCEOHRModal: React.FC<RouteToCEOHRModalProps> = ({ requestId, onSuccess, onClose }) => {
  ...
  useEffect(() => {
    itWorkflowService.getUsersByRole('CEO')
      .then(users => { setCeos(users); setFiltered(users); })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      setError(null);
      await approvalService.routeToCEO(requestId, selectedId, comments || undefined);
      onSuccess();
    } catch (err: any) {
      setError(err.response?.data?.message || 'Failed to route to CEO');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[9999] p-4" onClick={handleBackdropClick}>
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
          <div className="flex items-center gap-3 p-5 border-b border-gray-100">
            ...
            <div>
              <h2 className="font-bold text-base text-gray-900">Route to CEO for Approval</h2>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex justify-end gap-2 p-5 border-t border-gray-100 bg-gray-50 rounded-b-2xl">
              ...
              <button
                ...
                className="px-4 py-3 text-sm font-bold text-white bg-purple-600 rounded-lg hover:bg-purple-700 disabled:opacity-50"
              >
...
export default RouteToCEOHRModal;
