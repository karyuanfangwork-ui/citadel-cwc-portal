...
const CompleteDeliveryModal: React.FC<CompleteDeliveryModalProps> = ({ requestId, onSuccess, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      setError(null);
      await itWorkflowService.completeDelivery(requestId, notes || undefined);
      onSuccess();
    } catch (err: any) {
      setError(err.response?.data?.error || 'Failed to complete delivery');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[9999] p-4" onClick={handleBackdropClick}>
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
          <div className="flex items-center gap-3 p-5 border-b border-gray-100 bg-green-50">
            ...
            <div>
              <h2 className="font-bold text-base text-gray-900">Complete Delivery</h2>
              <p className="text-xs text-gray-500">IT Workflow · Confirm hardware delivered to requester</p>
            </div>
          </div>
          <form onSubmit={handleSubmit}>
            <div className="p-5 space-y-4">
              <p className="text-sm text-gray-600">
                This will resolve the request and notify the requester. Please add any delivery notes before confirming.
              </p>
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="e.g. Delivered monitor to desk 3-14. Asset tag logged as IT-0088."
