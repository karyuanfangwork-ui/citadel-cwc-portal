// frontend/src/components/request-detail/DceoDecisionFinModal.tsx
// Dedicated DCEO Decision modal for Finance Purchase Requisition workflow.
// Shows the attached invoice preview (if any) before the DCEO confirms/rejects.
...
interface DceoDecisionFinModalProps {
  requestId: string;
...
const DceoDecisionFinModal: React.FC<DceoDecisionFinModalProps> = ({
  requestId,
  attachments = [],
  onSuccess,
  onClose,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      setError(null);
      await financeWorkflowService.dceoDecision(requestId, decision, comments || undefined);
      onSuccess();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[9999] p-4" onClick={handleBackdropClick}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col max-h-[90vh]">
        <div className="flex items-center gap-3 p-5 border-b border-gray-100 bg-purple-50">
          ...
          <div>
            <h2 className="font-bold text-base text-gray-900">DCEO Decision</h2>
...
export default DceoDecisionFinModal;