// frontend/src/components/request-detail/CtoDecisionModal.tsx
...
const CtoDecisionModal: React.FC<CtoDecisionModalProps> = ({
  requestId,
  onSuccess,
  onClose,
}) => {
  ...
  const handleApprove = async () => {
    try {
      setSubmitting(true);
      setError(null);
      await itWorkflowService.ctoDecision(requestId, 'APPROVED', comments || undefined);
      onSuccess();
    } catch (err: any) {
  ...
  const handleReject = async () => {
    try {
      setSubmitting(true);
      setError(null);
      await itWorkflowService.ctoDecision(requestId, 'REJECTED', comments || undefined);
      onSuccess();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[9999] p-4" onClick={handleBackdropClick}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center gap-3 p-5 border-b border-gray-100 bg-amber-50">
          ...
          <div>
            <h2 className="font-bold text-base text-gray-900">CTO Approval</h2>
            <p className="text-xs text-gray-500">IT Workflow · CTO Decision Required</p>
          </div>
        </div>
        <form>
