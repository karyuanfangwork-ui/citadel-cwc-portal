...
export default function EdgeInspector({ edge, readOnly, onChange, onDelete }: EdgeInspectorProps) {
  const [agents, setAgents] = useState<{ id: string; firstName: string; lastName: string; email: string; agentTeam: string | null }[]>([]);

  useEffect(() => {
    if (!edge) return;
    void adminService.listAgents().then(setAgents).catch(() => setAgents([]));
  }, [edge?.id]);
  ...
  return <div className="space-y-4">
    ...
    <label className="block text-xs font-bold uppercase tracking-wide text-[#8993a4]">Auto-assign specific agent<select aria-label="Auto-assign specific agent" className="mt-1 w-full rounded-lg border border-[#b9c8de] bg-white px-3 py-2 text-sm" disabled={readOnly} value={edge.autoAssignUserId ?? ''} onChange={(event) => onChange({ autoAssignUserId: event.target.value || null })}>
      <option value="">No specific agent</option>
      {agents.map((agent) => <option key={agent.id} value={agent.id}>{agent.firstName} {agent.lastName}{agent.agentTeam ? ` · ${agent.agentTeam}` : ''}</option>)}
    </select></label>
    <p className="text-xs text-[#8993a4]">A specific agent takes priority over the role. Leave the role blank when routing to one named agent.</p>
