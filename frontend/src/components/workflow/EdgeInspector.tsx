...
const TRANSITION_LABELS = ['SUBMIT', 'APPROVE', 'REJECT', 'ADVANCE', 'RETURN', 'ESCALATE', 'CLOSE'] as const;
export default function EdgeInspector({ edge, readOnly, onChange, onDelete }: EdgeInspectorProps) {
  ...
  const transitionOptions = edge.transitionLabel && !TRANSITION_LABELS.includes(edge.transitionLabel as typeof TRANSITION_LABELS[number])
    ? [edge.transitionLabel, ...TRANSITION_LABELS]
    : TRANSITION_LABELS;
  return <div className="space-y-4">
    <p className="text-xs font-semibold text-[#8993a4]">{edge.fromNodeId} → {edge.toNodeId}</p>
    <label className="block text-xs font-bold uppercase tracking-wide text-[#8993a4]">Transition label<select aria-label="Transition label" className="mt-1 w-full rounded-lg border border-[#b9c8de] bg-white px-3 py-2 text-sm" disabled={readOnly} value={edge.transitionLabel ?? ''} onChange={(event) => onChange({ transitionLabel: event.target.value || null })}><option value="">No label</option>{transitionOptions.map((label) => <option key={label} value={label}>{label}</option>)}</select></label>
