...
vi.mock('../../../../../pages/credit/tabs/sections/RetailFacilitiesTab', () => ({ default: () => <div>Facilities editor</div> }));
...
describe('ApplicationPartiesWorkspace', () => {
  ...
  it('renders Facilities through the dedicated facilities editor', () => { renderWorkspace('facilities'); expect(screen.getByText('Facilities')).toBeInTheDocument(); expect(screen.getByText('Facilities editor')).toBeInTheDocument(); expect(screen.queryByText('Application editor')).not.toBeInTheDocument(); });
