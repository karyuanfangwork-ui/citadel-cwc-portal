...
const individualProfile = (overrides: Partial<BorrowerProfile> = {}): BorrowerProfile => ({
  ...
  netWorth: null, registrationNumber: null, industry: null, nricPassport: '900101-01-1234', address: null,
  phone: null, email: null, isActive: true, kycVerifiedAt: null, deletedAt: null,
...
const summary = (overrides: Partial<Borrower360Summary> = {}): Borrower360Summary => ({
  ...
  income: { gross: 10000, commitments: 1000, netIncome: 9000 }, bureauFacilities: [], alerts: [], ...overrides,
});
...
const summaryWithStaleBureau = () => summary({ bureau: { source: 'CTOS', uploadedAt: '2026-07-01T00:00:00.000Z', daysOld: 120, stale: true } });
const summaryWithMissingIncomeAndStaleBureau = () => summary({ income: null, bureau: { source: 'CTOS', uploadedAt: '2026-07-01T00:00:00.000Z', daysOld: 120, stale: true } });
const verifiedIndividual = () => individualProfile({ kycVerifiedAt: '2026-07-10T00:00:00.000Z' });
const readyIndividual = () => individualProfile({ kycVerifiedAt: '2026-07-10T00:00:00.000Z', creditRiskRating: 'A' });
const readySummary = () => summary({ riskRating: { effective: 'A', base: 'A', calculatedAt: '2026-08-01T00:00:00.000Z', version: 1, reasonCodes: [], missingInputs: [], bureauCapsApplied: [] } });

it('blocks a borrower with missing KYC, income, and stale bureau data', () => {
  const result = calculateBorrowerReadiness({ profile: individualProfile(), summary: summaryWithMissingIncomeAndStaleBureau(), applications: [] });
...
it('requires financial information for corporate borrowers', () => {
  const profile = individualProfile({ borrowerType: 'CORPORATE', name: 'Acme Sdn Bhd', nricPassport: null, registrationNumber: '202001234567', kycVerifiedAt: '2026-07-10T00:00:00.000Z' });
