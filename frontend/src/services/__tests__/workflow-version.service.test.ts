...
describe('workflowVersionService', () => {
  ...
  it('calls validation and lifecycle endpoints', async () => { apiClient.post.mockResolvedValue(ok({ validation: { blocking: [], warnings: [] }, remapPlan: { entries: [], totalRequests: 0 } })); await workflowVersionService.validateVersion('v'); expect(apiClient.post).toHaveBeenCalledWith('/admin/workflows/versions/v/validate'); apiClient.post.mockResolvedValueOnce(ok({ version: 2, transitionCount: 0, stepCount: 0, movedCount: 0 })); await workflowVersionService.publishVersion('v'); expect(apiClient.post).toHaveBeenCalledWith('/admin/workflows/versions/v/publish', { statusRemap: {} }); apiClient.post.mockResolvedValueOnce(ok({ version: 1 })); await workflowVersionService.rollbackVersion('v'); expect(apiClient.post).toHaveBeenCalledWith('/admin/workflows/versions/v/rollback'); });
  ...
  it('posts the mapping when one is supplied', async () => {
    apiClient.post.mockResolvedValue(ok({ version: 4, transitionCount: 1, stepCount: 2, movedCount: 2 }));
    const result = await workflowVersionService.publishVersion('v4', { LEGACY: 'IN_PROGRESS' });
    expect(apiClient.post).toHaveBeenCalledWith('/admin/workflows/versions/v4/publish', { statusRemap: { LEGACY: 'IN_PROGRESS' } });
    expect(result.movedCount).toBe(2);
  });

  it('posts an empty mapping when none is supplied', async () => {
    apiClient.post.mockResolvedValue(ok({ version: 4, transitionCount: 1, stepCount: 2, movedCount: 0 }));
    await workflowVersionService.publishVersion('v4');
    expect(apiClient.post).toHaveBeenCalledWith('/admin/workflows/versions/v4/publish', { statusRemap: {} });
  });
});
